import type { SubscriptionClient, SubscriptionClientConnection, Unsubscribe } from 'remult'
import { type RemultPartyMessage, resolveRoomIdFromChannel } from './channel.js'
...
interface PoolEntry {
	subscribe(channel: string, onMessage: MessageHandler, onError: ErrorHandler): Unsubscribe
	publish(channel: string, payload: unknown): void
	dispose(): void
}

// A single WebSocket pool for one room. The underlying socket stays open across async
// boundaries, re-subscribes active channels after reconnect, and tears down after a
// grace period once the refcount hits zero.
function createPoolEntry(
	...
	const subscribeChannel = (channel: string) => {
		...
		const pending = retryTimers.get(channel)
		if (pending) clearTimeout(pending)
		retryTimers.set(
			channel,
			setTimeout(() => {
				retryTimers.delete(channel)
				if (!acked.has(channel) && !disposed) {
					send({ type: 'remult:subscribe', channel })
					retryTimers.set(
						channel,
						setTimeout(() => {
							retryTimers.delete(channel)
							if (!acked.has(channel) && !disposed) {
								console.warn('[realtime] subscribe not acked for', channel)
							}
						}, 800),
					)
				}
	...
	const handleMessage = (event: MessageEvent) => {
		...
		try {
			...
			if (msg.channel && msg.type === 'data') {
				...
				acked.add(msg.channel)
				const t = retryTimers.get(msg.channel)
				if (t) {
					clearTimeout(t)
					retryTimers.delete(msg.channel)
				}
...
// Remult SubscriptionClient with one WS pool per room (DO partition). Each room gets a
// single connection with transparent reconnect; the server broadcasts to the room.
export class RemultPartySubscriptionClient implements SubscriptionClient {
	...
	async openConnection(onReconnect: () => void): Promise<SubscriptionClientConnection> {
		return {
			subscribe: async (channel, onMessage, onError) => {
				const roomId = this.options.resolveRoomId?.(channel) ?? resolveRoomIdFromChannel(channel)
				let pool = this.pools.get(roomId)
				if (!pool) {
					pool = createPoolEntry(roomId, this.options, onReconnect, () => this.pools.delete(roomId))
	...
	publish(channel: string, payload: unknown): void {
		const roomId = this.options.resolveRoomId?.(channel) ?? resolveRoomIdFromChannel(channel)
		this.pools.get(roomId)?.publish(channel, payload)
	}
}
