...
say ":: svelte-kit sync…"
(cd apps/frontend && bunx svelte-kit sync) 2>&1 | sed 's/^/   /'
say ""

# ---- hard gates ----
run "frontend svelte-check" bash -c "cd apps/frontend && bunx svelte-check --tsconfig ./tsconfig.json"
run "backend tsc"           bash -c "cd apps/backend && bunx tsc -p tsconfig.json"
run "biome"                bunx biome check

# ---- fallow (advisory — surface hotspots, never gates the run) ----
say ":: fallow…"
bunx fallow >"$tmpd/flw_out" 2>"$tmpd/flw_err" || true
findings=$(grep -E '^\s*●' "$tmpd/flw_out" 2>/dev/null || true)
if [ -n "$findings" ]; then
	say "   fallow: findings (advisory — fix hotspots below)"
